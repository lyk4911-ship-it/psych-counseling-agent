...
# 阶段1：分析库 - 辅助构建用户画像
# 作用：模拟心理咨询师在谈话过程中构建用户画像的潜在逻辑，涉及专业心理学知识
# 分析库包含专业的评估工具和画像构建方法，帮助理解用户状态
ANALYSIS_SYS = (
...
# 阶段2：策略库 - 选择干预策略
# 作用：模拟心理咨询师在裁决阶段从认知中所有专业策略中选择最符合的那个
# 基于已构建的用户画像，从策略库中选择最合适的干预方法
STRATEGY_PICK_SYS = (
...
# 【替换】建议输出：框架优先，少任务清单；提问受「本轮约束」控制
PLAN_REPLY_SYS = (
...
# 【替换】/show：证据驱动会谈摘要，少模板腔、少「心理问题」套话
SHOW_SYS = (
...
# 模型失败或未过品控时仍给一句「像朋友」的兜底（与 AUTO_MESSAGES 区分开，略多几条轮换）
PROACTIVE_FALLBACKS = [
    "我在呢，你刚才说的我记着了。",
    "先喘口气也行，不急着马上想清楚。",
    "要是还想聊，我都在。",
...
STATE0 = {
  ...
  "turns": 0,
  "_history": [],
  "current_strategy": None,  # {strategy_id, why, confidence, ts}
  "last_plan": None,         # {strategy_id, ts, feedback(optional)}
  # 【新增】主动式调控 / 提问门槛（agent_rules 使用）
  "conversation_mode": "emotion",  # info | career | emotion | risk
...
# =============== 阶段1：画像增量更新（使用分析库） =============
# ======================================================
# 分析库作用：提供专业的心理学评估方法和工具，辅助LLM理解用户状态并构建画像
# 模拟咨询师在谈话过程中构建用户画像的潜在逻辑
...
# =============== 阶段2：策略选择（使用策略库） =========================
# ======================================================
# 策略库作用：提供所有可用的干预策略，基于画像选择最合适的策略
# 模拟咨询师在裁决阶段从认知中所有专业策略中选择最符合的那个
...
def _run_proactive_friend_line(state: Dict[str, Any]) -> Tuple[str, bool]:
    """生成一条「好友式」主动消息。返回 (reply, skipped)。"""
    now = time.time()
...
class DialogAgent:
    ...
    def run(
        ...
        # /show：仅测试者/后台接口允许；用户端应传 allow_show_report=False
        ...
        # ---------- 阶段1：画像增量更新（使用分析库辅助构建画像） ----------
        # 分析库提供专业的心理学评估方法，帮助理解用户状态
        ...
        # ---------- 阶段2：周期性策略选择（使用策略库选择最合适的干预策略） ----------
        # 基于已构建的画像，从策略库中选择最合适的策略（后台可见，不一定给用户展示）
        ...
        if reply is None:
            ...
            # 添加对话历史（包括当前user，因为已经在704行加入history了）
            hist_msgs = _history_to_messages(state)
            if len(hist_msgs) > 0:
                # 限制历史长度，保留最近10轮对话（20条消息），避免token过多
                if len(hist_msgs) > 20:
